...
int segmentation_roi(cv::Mat& roi, cv::Mat& mask, int feature, std::vector< int >& param)
{
    ...
    Mat visited = ~mask;// visited map: used in dfs. It is reserve of mask
    ...
    printoutPath(roi,allpaths,true);
...
void printoutPath(Mat& img,vector<PathInfo>& allpath,bool only_show_best_path)
{
    ...
    if(only_show_best_path){
        ...
        putText(img,"socre "+to_string(score),Point(0,0.5*img.rows),FONT_HERSHEY_SCRIPT_SIMPLEX,2,Scalar(255),4);
