...
int main(int argc, char **argv) {
  ...
  double mean_radius =  accumulate(begin(radiuses),end(radiuses), 0.0)/radiuses.size();
...
void estimateCenterDistance(Mat& img, vector< Point > centers, double radius, Point* bl, double* mean_object_dist, double* angle)
{
  ...
  //  double object_space[2];
    ...
    //圆心距
     *mean_object_dist = accumulate(begin(object_dist),end(object_dist), 0.0)/object_dist.size();
